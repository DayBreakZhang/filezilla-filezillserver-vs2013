...
BOOL CUsersDlgSpeedLimit::DisplayUser(const t_user *pUser)
{
	...
	{
		m_DownloadValue = pUser->nSpeedLimit[download];
		m_UploadValue = pUser->nSpeedLimit[upload];
		m_DownloadSpeedLimitType = pUser->nSpeedLimitType[download];
		m_UploadSpeedLimitType = pUser->nSpeedLimitType[upload];
		...
		{
			((CButton *)GetDlgItem(IDC_USERS_SPEEDLIMIT_SERVERBYPASS_DOWNLOAD))->SetButtonStyle(BS_AUTO3STATE);
			((CButton *)GetDlgItem(IDC_USERS_SPEEDLIMIT_SERVERBYPASS_UPLOAD))->SetButtonStyle(BS_AUTO3STATE);
		}

		((CButton *)GetDlgItem(IDC_USERS_SPEEDLIMIT_SERVERBYPASS_DOWNLOAD))->SetCheck(pUser->nBypassServerSpeedLimit[download] % 3);
		((CButton *)GetDlgItem(IDC_USERS_SPEEDLIMIT_SERVERBYPASS_UPLOAD))->SetCheck(pUser->nBypassServerSpeedLimit[upload] % 3);
	}

...
void CGroupsDlgSpeedLimit::OnSpeedlimitDownloadUp()
{
	...
	if ( ( curSel >= 1) && ( curSel < (int)m_DownloadSpeedLimits.size()))
	{
...
void CGroupsDlgSpeedLimit::OnSpeedlimitDownloadDown()
{
	...
	if ( ( curSel >= 0) && ( curSel < (int)m_DownloadSpeedLimits.size() - 1))
	{
		CSpeedLimit sl = m_DownloadSpeedLimits[curSel];
		m_DownloadSpeedLimits[curSel] = m_DownloadSpeedLimits[curSel + 1];
		m_DownloadSpeedLimits[curSel + 1] = sl;
...
void CGroupsDlgSpeedLimit::OnSpeedlimitUploadUp()
{
	...
	if ( ( curSel >= 1) && ( curSel < (int)m_UploadSpeedLimits.size()))
	{
...
void CGroupsDlgSpeedLimit::OnSpeedlimitUploadDown()
{
	...
	if ( ( curSel >= 0) && ( curSel < (int)m_UploadSpeedLimits.size() - 1))
	{
		CSpeedLimit sl = m_UploadSpeedLimits[curSel];
		m_UploadSpeedLimits[curSel] = m_UploadSpeedLimits[curSel + 1];
		m_UploadSpeedLimits[curSel + 1] = sl;
...
BOOL CGroupsDlgSpeedLimit::DisplayGroup(const t_group *pGroup)
{
	...
	{
		m_DownloadValue = pGroup->nSpeedLimit[download];
		m_UploadValue = pGroup->nSpeedLimit[upload];
		m_DownloadSpeedLimitType = pGroup->nSpeedLimitType[download] - 1;
...
BOOL CGroupsDlgSpeedLimit::SaveGroup(t_group *pGroup)
{
	...
	if (m_UploadValue > 65535)
		m_UploadValue = 65535;
	pGroup->nSpeedLimit[download] = m_DownloadValue;
	pGroup->nSpeedLimit[upload] = m_UploadValue;
	pGroup->nSpeedLimitType[download] = m_DownloadSpeedLimitType + 1;
	pGroup->nSpeedLimitType[upload] = m_UploadSpeedLimitType + 1;

	pGroup->SpeedLimits[download] = m_DownloadSpeedLimits;
	pGroup->SpeedLimits[upload] = m_UploadSpeedLimits;

	pGroup->nBypassServerSpeedLimit[download] = ((CButton *)GetDlgItem(IDC_GROUPS_SPEEDLIMIT_SERVERBYPASS_DOWNLOAD))->GetCheck() ? 1 : 0;
	pGroup->nBypassServerSpeedLimit[upload] = ((CButton *)GetDlgItem(IDC_GROUPS_SPEEDLIMIT_SERVERBYPASS_UPLOAD))->GetCheck() ? 1 : 0;
	return TRUE;
}
